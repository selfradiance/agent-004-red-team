// Integration tests for protocol abuse attack scenarios — requires live AgentGate

import "dotenv/config";
import { describe, it, expect, beforeAll } from "vitest";
import { generateKeyPairSync, randomUUID } from "node:crypto";
import { protocolAttacks } from "../../src/attacks/protocol";
...
describe.skipIf(!process.env.AGENTGATE_REST_KEY || process.env.AGENTGATE_REST_KEY.includes("your-"))(
  "protocol attacks — live AgentGate",
  () => {
    ...
    it("8.1 — malformed request shape is rejected", async () => {
      const result = await protocolAttacks[0].execute(client);
      expect(result.caught).toBe(true);
      expect(result.scenarioId).toBe("8.1");
    });

    it("8.2 — wrong content-type is handled", async () => {
      const result = await protocolAttacks[1].execute(client);
      expect(result.caught).toBe(true);
      expect(result.scenarioId).toBe("8.2");
    });
  },
);
