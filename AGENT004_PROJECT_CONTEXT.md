...
- **Swarm mode has no novel attack / sandbox integration** — novel attack generation and the 4-layer sandbox (validator → executor → child-runner → toolkit-host) only run in `--static` and `--team` modes. Swarm mode (`--swarm`) runs only registry attacks and Beta tasks, which are hardcoded. If novel attacks are added to swarm mode in the future, they'll need explicit sandbox integration.
- **JSON response parsers handle common backtick fencing but not edge cases** — all Claude response parsers strip `\`\`\`json...\`\`\`` fences, but would fail on nested fences or unusual whitespace before the fence. Mitigated: all swarm parsers fall back to default actions on parse failure, so this only causes a fallback rather than a crash.
...
1. ~~Write the Medium article for Agent 003 (Email Rewriter)~~ — DONE. Published.
2. ~~Write the base Medium article for Agent 004~~ — DONE. Published to Medium and Twitter. Title: "What Happens When an AI Agent Tries to Break the System That Governs It?" Covers Stages 1–4 (static → adaptive → recursive → coordinated team). Published March 2026.
3. Write the Agent 004 Swarms article — v0.5.0 Stage 5 content. The swarm campaign (9 agents, 3 teams, 5-round interleaved campaigns, coordinator synthesis). Lead with "we scaled from 3 coordinated agents to 9-agent swarms." Feature: 97 attacks in canonical run, 0 swarm-emergent findings, AgentGate held. Technically deep, Hacker News candidate. NOTE: May hold this until after Agent 005 article ships, then consider whether to also run swarms on Agent 005 before writing the swarms piece.
4. Update AgentGate README with Agent 004 v0.5.0 in "Built With AgentGate" section.
5. Consider Agent 005 article — Recursive Code Reviewer. Reuse the sandbox architecture from Agent 004 in a constructive context. Strongest framing: "I took the sandbox that survived 100+ adversarial tests and turned it into something constructive."
6. Burden for further red-team escalation is high — v0.5.0 canonical run found 0 swarm-emergent findings and the 8-round + Codex audits surfaced no critical or high issues. Further escalation (more agents, more teams, longer campaigns) would likely produce diminishing returns without AgentGate adding reputation-gated features to exploit. Agent 004 is functionally complete.
7. New idea under evaluation: Identity-and-accountability layer for AI agents (working names: AgentPass, Bonded Delegation, Agent Warrant). Inspired by CSA March 2026 study showing 68% of orgs can't distinguish human vs. agent activity. Hardware-backed human delegation (YubiKey/FIDO2) + bonded action settlement. Three-way audit completed (ChatGPT, Gemini, Grok) — consensus: build as standalone proof after Agent 004 swarms article, not as AgentGate extension. New repo, new name. See audit document for full details.
